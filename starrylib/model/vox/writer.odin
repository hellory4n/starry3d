...
)
{
	...
	stlib.write_int_to_file(file, Int(150)) or_return

	// main chunk
	os.write_string(file, "MAIN") or_return
	stlib.write_int_to_file(file, Int(0)) or_return
	file_size_position := stlib.file_position(file) or_return // set later
	stlib.write_int_to_file(file, Int(0)) or_return

	// size chunk
	os.write_string(file, "SIZE") or_return
	stlib.write_int_to_file(file, Int(size_of(Int) * 3)) // chunk size
	stlib.write_int_to_file(file, Int(0)) // children chunk size
	stlib.write_int_to_file(file, Int(m.size.x))
	stlib.write_int_to_file(file, Int(m.size.y))
	stlib.write_int_to_file(file, Int(m.size.z))

	// xyzi chunk
	os.write_string(file, "XYZI") or_return

	stlib.write_int_to_file(file, Int(m.voxel_count * 4)) // chunk size
	stlib.write_int_to_file(file, Int(0)) // children chunk size
	stlib.write_int_to_file(file, Int(m.voxel_count))
	...
	for z in m.start.z ..< m.end.z {
		for y in m.start.y ..< m.end.y {
			for x in m.start.x ..< m.end.x {
				...
				stlib.write_int_to_file(file, Char(x - m.start.x))
				stlib.write_int_to_file(file, Char(z - m.start.z))
				stlib.write_int_to_file(file, Char(y - m.start.y))
				stlib.write_int_to_file(file, Char(closest_color_idx(val)))
			}
		}
	}
	...
	stlib.write_int_to_file(file, Int(0)) // chunk size
	stlib.write_int_to_file(file, Int(0)) // children chunk size

	// now we know the size of the rest of the file
	main_chunk_children_size := (stlib.file_position(file) or_return) - file_size_position
	os.seek(file, file_size_position, .Start)
	stlib.write_int_to_file(file, Int(main_chunk_children_size)) or_return
