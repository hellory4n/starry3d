...
void main()
{
	...
	vec3 position = vec3(chunk) * CHUNK_SIZE + vec3(v.position);

	// procedurally generated quad (why not (why))
	vec3 quad_position;
	switch (gl_VertexID) {
	case 0:
	...
	gl_Position = u_projection * u_view * vec4(position + quad_position, 1.0);
