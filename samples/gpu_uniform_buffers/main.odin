...
// remember to follow std140 alignment rules
Uniforms :: struct #align (16) #max_field_align(16) {
...
render_app :: proc(dt: f32, dev: gpu.Device)
{
	...
	gpu.update_buffer(dev, app.uniform_buffer, mem.ptr_to_bytes(&uniforms))
	gpu.bind_uniform_buffer(dev, app.uniform_buffer, slot = 0)
...
main :: proc()
{
	...
	stapp.run(
		app_name = "gpu uniform buffers",
		app_version = {0, 1, 0},
		asset_dir = "samples/gpu_uniform_buffers",
		init_proc = new_app,
		free_proc = free_app,
