...
texture_size :: proc(h: stapp.Asset_Handle) -> [2]i32
{
...
texture_channels :: proc(h: stapp.Asset_Handle) -> Texture_Channels
{
	texture, ok := hm.get(&global.textures, h.handle)
	assert(ok)
	assert(h.type == ASSET_TEXTURE)
...
texture_bit_depth :: proc(h: stapp.Asset_Handle) -> int
{
	texture, ok := hm.get(&global.textures, h.handle)
	assert(ok)
	assert(h.type == ASSET_TEXTURE)
...
texture_data :: proc(h: stapp.Asset_Handle) -> []byte
{
	texture, ok := hm.get(&global.textures, h.handle)
	assert(ok)
	assert(h.type == ASSET_TEXTURE)
...
texture_gpu_handle :: proc(h: stapp.Asset_Handle) -> gpu.Texture
{
	texture, ok := hm.get(&global.textures, h.handle)
	assert(ok)
	assert(h.type == ASSET_TEXTURE)
