...
texture_size :: proc(h: stapp.Asset_Ref) -> [2]i32
{
	texture, ok := hm.get(&global.textures, h.handle)
	assert(ok)
	assert(h.type == st.strid("texture"))
...
texture_channels :: proc(h: stapp.Asset_Ref) -> Texture_Channels
{
	texture, ok := hm.get(&global.textures, h.handle)
	assert(ok)
	assert(h.type == st.strid("texture"))
...
texture_bit_depth :: proc(h: stapp.Asset_Ref) -> int
{
	texture, ok := hm.get(&global.textures, h.handle)
	assert(ok)
	assert(h.type == st.strid("texture"))
...
texture_data :: proc(h: stapp.Asset_Ref) -> []byte
{
	texture, ok := hm.get(&global.textures, h.handle)
	assert(ok)
	assert(h.type == st.strid("texture"))
...
texture_gpu_handle :: proc(h: stapp.Asset_Ref) -> gpu.Texture
{
	texture, ok := hm.get(&global.textures, h.handle)
	assert(ok)
	assert(h.type == st.strid("texture"))
